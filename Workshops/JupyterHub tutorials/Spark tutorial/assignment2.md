...
rating = rating.filter(lambda x: x != firstline).map(lambda x: tuple(x.split(',')[:3]))
rating.take(5)
```
...
userbudget = userbudget.filter(lambda x: x != firstline2).map(lambda x: (x.split(',')[0], x.split(',')[-2]))
userbudget.take(5)
```
...
```python
userrating = rating.map(lambda x: (x[0], x[2]))
userrating.take(5)
```
...
```python
userbudget_rating = userbudget.join(userrating).map(lambda x: (x[1][0], int(x[1][1])))
userbudget_rating.take(5)
```
...
```python
userbudget_rating.groupByKey().map(lambda x: (x[0], sum(x[1]) / len(x[1]))).collect()
```
...
userpos = userpos.filter(lambda x: x != firstline3).map(lambda x: (x.split(',')[0], (x.split(',')[1], x.split(',')[2])))
userpos.take(5)
```
...
```python
userposdict = sc.broadcast(dict(userpos.collect()))
userposdict.value
```
...
placepos = placepos.filter(lambda x: x != firstline4).map(lambda x: (x.split(',')[0], (x.split(',')[1], x.split(',')[2])))
placepos.take(5)
```
...
```python
placeposdict = sc.broadcast(dict(placepos.collect()))
placeposdict.value
```
...
```python
rating_distance = rating.map(lambda x: (x[2], vincenty(userposdict.value[x[0]], placeposdict.value[x[1]]).meters))
rating_distance.take(5)
```
...
```python
rating_distance.groupByKey().map(lambda x: (x[0], sum(x[1]) / len(x[1]))).collect()
```
