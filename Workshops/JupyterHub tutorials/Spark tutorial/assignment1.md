...
```python
babynames.filter(lambda x: x[3] == 'M').map(lambda x: int(x[4])).sum()
```
...
```python
babynameskv = babynames.map(lambda x: (x[1], 1))
babynameskv.take(5)
```
...
```python
babynamestotal = babynameskv.reduceByKey(lambda x, y: x + y)
babynamestotal.take(5)
```
...
```python
babynamestotal.map(lambda x: x[1]).max()
```
...
```python
babynamestotal.filter(lambda x: x[1] == 386).collect()
```
